...
type Query func(ctx context.Context, peer, addr *Address) any
...
func (s *LookupService) LookupNode(ctx context.Context, addr *Address, timeout time.Duration) (entry *Endpoint, err error) {
	...
	query := func(ctx context.Context, peer, addr *Address) any {
		// perform query
...
func (s *LookupService) Lookup(ctx context.Context, addr *Address, resolver Query, timeout time.Duration) (res any, err error) {
	...
	var query func(peer, addr *Address, ch chan any)
	queryPeers := func(peers []*Address, addr *Address, ch chan any) {
	...
	query = func(peer, addr *Address, ch chan any) {
		if !running {
			return
		}
